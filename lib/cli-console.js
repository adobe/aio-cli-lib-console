...
class ConsoleCLILib {
   ...
   * @memberof ConsoleCLILib
   */
  constructor (consoleClient, consoleCredentials, spinner, options) {
   ...
   * @returns {Promise<ConsoleCLILib>} new ConsoleCLILib instance
   * @memberof ConsoleCLILib
   */
  static async init (consoleCredentials, spinner, options = {}) {
    const consoleClient = await consoleSdk.init(
      consoleCredentials.accessToken,
      consoleCredentials.apiKey,
      consoleCredentials.env
    )
    return new ConsoleCLILib(consoleClient, consoleCredentials, spinner, options)
  }

  async getOrganizations () {
    this.spinner.start()
    this.spinner.text = 'Getting Organizations...'
  ...
  async promptForSelectOrganization (organizations, data = { orgId: null }) {
    let selectedOrg
    // pre-selection
    if (data.orgId) {
  ...
  async getProjects (orgId) {
    this.spinner.start()
    this.spinner.text = 'Getting Projects...'
  ...
  async promptForSelectProject (projects, data = { projectId: null }, options = { allowCreate: null }) {
    let selectedProject
    if (data.projectId) {
  ...
  async getWorkspaces (orgId, projectId) {
    this.spinner.start()
    this.spinner.text = 'Getting Workspaces...'
  ...
  async promptForSelectWorkspace (workspaces, data = { workspaceId: null }, options = { allowCreate: true }) {
    ...
    if (data.workspaceId) {
      ...
      // Note: promptSelectOrCreate gives the user the possibility to escape the
      // selection, in which case the selected workspace will be null
      const promptFunc = options.allowCreate ? prompt.promptSelectOrCreate : prompt.promptSelect
  ...
  async promptForCreateProjectDetails () {
    // todo support predefined data input
    ...
    logger.debug(`Selected Project Details: ${JSON.stringify(projectDetails, null, 2)}`)
    return projectDetails
  }

  /**
   * @param orgId
   * @param createProjectDetails
   */
  async createProject (orgId, createProjectDetails) {
    ...
    this.spinner.text = 'Creating Project...'
  ...
  async promptForCreateWorkspaceDetails () {
    ...
    logger.debug(`Selected Workspace Details: ${JSON.stringify(workspaceDetails, null, 2)}`)
  ...
  async createWorkspace (orgId, projectId, createWorkspaceDetails) {
    ...
    this.spinner.text = 'Creating Workspace...'
    ...
    // get complete record
    this.spinner.text = 'Getting new Workspace...'
  ...
  async getEnabledServicesForOrg (orgId) {
    this.spinner.start('Retrieving services supported by the Organization...')
  ...
  async promptForServiceProperties (workspaceName, supportedServices) {
    ...
    const selectedServices = await prompt.promptMultiSelect(
      `Add Services to new Workspace '${workspaceName}'`,
      serviceChoices
    )
  ...
  async addServicesToWorkspace (orgId, project, workspace, certDir, serviceProperties) {
    ...
    if (serviceProperties.length <= 0) {
      // make sure there are any services attached before doing anything
      return
    }
    ...
    this.spinner.start(`Attaching Services to the Enterprise Credentials of Workspace ${workspace.name}...`)
  ...
  async getServicePropertiesFromWorkspace (orgId, projectId, workspace, supportedServices) {
    const credential = await this.getFirstEntpCredentials(orgId, projectId, workspace)
    if (!credential) {
      return []
    }
    this.spinner.start(`Getting Services attached to the Workspace ${workspace.name} Credentials`)
    ...
    this.spinner.stop()
    logger.debug(`(Fixed) Service Properties for Workspace ${workspace.id}: ${JSON.stringify(fixedServiceProperties, null, 2)}`)
    return fixedServiceProperties
  }

  async confirmAddServicesToWorkspace (workspaceName, serviceProperties) {
    const serviceNames = helpers.servicePropertiesToNames(serviceProperties)
    console.log(`Services to be added:\n${JSON.stringify(serviceNames, null, 4)}`)
    const confirmSubscription = await prompt.promptConfirm(
      `  > do you confirm adding these services to the Workspace ${workspaceName} ?`
    )
    logger.debug(`Will add services: ${confirmSubscription}`)
    return confirmSubscription
  }

  async createEnterpriseCredentials (orgId, project, workspace, certDir, options = { certValidDays: 365 }) {
    const {
      projectCertDir,
      publicKeyFileName,
      publicKeyFilePath,
    ...
    this.spinner.start(`Generating Credential key pair for Workspace ${workspace.name}...`)
    const { cert, privateKey } = certPlugin.generate(orgId + project.name + workspace.name, options.certValidDays)
    fs.ensureDirSync(projectCertDir)
    fs.writeFileSync(publicKeyFilePath, cert)
    fs.writeFileSync(privateKeyFilePath, privateKey)
    this.spinner.stop()
    console.log(`Key pair '${publicKeyFileName}, ${privateKeyFileName}' valid for '${options.certValidDays}' days, has been created into the folder: ${projectCertDir}`)

    this.spinner.start(`Creating Enterprise Credentials for Workspace ${workspace.name}...`)
  ...
  async getFirstEntpCredentials (orgId, projectId, workspace) {
    this.spinner.start(`Getting Enterprise Credentials attached to Workspace ${workspace.name}...`)
  ...
  async getWorkspaceConfig (orgId, projectId, workspaceId, supportedServices) {
    this.spinner.start('Downloading Workspace config...')
    const json = (await this.sdkClient.downloadWorkspaceJson(orgId, projectId, workspaceId)).body
    this.spinner.stop()
    // enhance configuration with supported services
    json.project.org.details = {
      ...json.project.org.details,
      services: supportedServices.map(s => ({ name: s.name, code: s.code, type: s.type }))
    }
  ...
  async promptForAddServicesOperation (workspaceName, options = { cloneChoice: false, nopChoice: true }) {
    if (!options.cloneChoice && !options.exitChoice) {
      return 'add'
    }
    const choices = helpers.getAddServicesOperationPromptChoices(options)
    const choice = await prompt.promptSelect(
      `Do you wish to attach new Services to the Workspace ${workspaceName}?`,
      choices,
      {}
    )
