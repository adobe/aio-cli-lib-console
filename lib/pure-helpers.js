...
const loggerNamespace = '@adobe/aio-cli-lib-console/pure-helpers'
...
function fixServiceProperties (serviceProperties, supportedServices) {
  return serviceProperties.map(sp => {
    ...
    if (!orgService) {
      logger.warn(`Supported services did not contain a service with matching sdkCode : ${sp.sdkCode}`)
    }
    let fixedLicenseConfigs = null
    if (sp.licenseConfigs && orgService && orgService.properties && orgService.properties.licenseConfigs) {
...
/** @private */
function mergeExtensionPoints (currPayload, updatePayload) {
  // make sure to not overwrite fields in incoming objects
  const currPayloadEndpoints = clonedeep(currPayload.endpoints)
  const updatePayloadEndpoints = clonedeep(updatePayload.endpoints)
  // set merged to currPayload
  const merged = currPayloadEndpoints
  // add new payload to merged
  Object.entries(updatePayloadEndpoints)
    .forEach(([endpointName, newEndpoint]) => {
      const oldEndpoint = currPayloadEndpoints[endpointName]
...
/** @private */
function removeExtensionPoints (currPayload, payloadToRemove) {
  const currPayloadEndpoints = clonedeep(currPayload.endpoints)
  const payloadToRemoveEndpoints = clonedeep(payloadToRemove.endpoints)
  const ret = currPayloadEndpoints
  Object.entries(payloadToRemoveEndpoints)
    .forEach(([endpointName, endpoint]) => {
      const endpointToRemoveFrom = ret[endpointName]
      Object.keys(endpoint).forEach(operationName => {
        if (endpointToRemoveFrom && endpointToRemoveFrom[operationName]) {
          delete endpointToRemoveFrom[operationName]
        }
      })
      if (endpointToRemoveFrom && Object.keys(endpointToRemoveFrom).length <= 0) {
        delete ret[endpointName]
      }
    })
...
/**
 * @typedef {object} DevTerms
 * @property {string} text Text of Developer Terms
 * @property {string} locale Language of the Developer Terms text
 */

/** @private
 *  @returns {DevTerms | {}} dev terms
 * */
function getDevTerms (payload) {
  if (payload && Array.isArray(payload.tc)) {
    return payload.tc[0]
  }
  return {}
}
...
module.exports = {
  ...
  findFirstOAuthServerToServerCredential,
