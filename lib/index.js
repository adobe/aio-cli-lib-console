...
class LibConsoleCLI {
   ...
   * @returns {Promise<Array<{name: string}>>} the feature list, or `[]` on error
   */
  async getOrganizationFeatures (orgId) {
    try {
      return (await this.sdkClient.getOrganizationFeatures(orgId)).body || []
    } catch (err) {
      logger.debug(`getOrganizationFeatures failed for ${orgId}: ${err}`)
      return []
    }
  }
  ...
  async hasOrgFeature (orgId, featureName, options = {}) {
    try {
      const features = (await this.sdkClient.getOrganizationFeatures(orgId)).body || []
