...
class ConsoleCLILib {
  ...
  static async init (consoleCredentials, options = {}) {
    ...
    return new ConsoleCLILib(consoleClient, consoleCredentials, options)
  }
  ...
  async getServicePropertiesFromWorkspace (orgId, projectId, workspace, supportedServices) {
    ...
    spinner.start(`Getting Services attached to the Workspace ${workspace.name} Credentials`)
  ...
  async getFirstEntpCredentials (orgId, projectId, workspace) {
    ...
    const credentialResponse = await this.sdkClient.getCredentials(orgId, projectId, workspace.id)
    spinner.stop()
    logger.debug('Get Credentials Response', JSON.stringify(credentialResponse.body, null, 2))
    return helpers.findFirstEntpCredential(credentialResponse.body)
  }
