...
class LibConsoleCLI {
  ...
  async promptForSelectOrganization (organizations, data = { orgId: undefined }) {
    let selectedOrg
    // pre-selection
    if (data.orgId) {
  ...
  async promptForSelectProject (projects, data = { projectId: undefined }, options = { allowCreate: false }) {
    let selectedProject
    if (data.projectId) {
  ...
  async promptForSelectWorkspace (workspaces, data = { workspaceId: undefined }, options = { allowCreate: false }) {
  ...
  async confirmAddServicesToWorkspace (workspaceNameOrNames, serviceProperties) {
    const what = helpers.workspaceNamesToPromptString(workspaceNameOrNames)
  ...
  > do you confirm adding these services to the ${what} ?`
    )
  ...
  async promptForAddServicesOperation (workspaceNameOrNames, options = { cloneChoice: false, nopChoice: true }) {
    const what = helpers.workspaceNamesToPromptString(workspaceNameOrNames)
    ...
    const choice = await prompt.promptSelect(
      `Do you wish to attach new Services to the ${what}?`,
      choices,
      {}
    )
