...
class LibConsoleCLI {
  ...
  async getProjectNextAvailableIdentifiers (orgId) {
    spinner.start('Getting next available project identifiers...')
    const data = (await this.sdkClient.getProjectNextAvailableIdentifiers(orgId)).body
    spinner.stop()
    logger.debug(`Get next available project identifiers response for Org ${orgId}: ${JSON.stringify(data, null, 2)}`)
    return data
  }
  ...
  async acceptDevTermsForOrg (orgId) {
    const res = await this.sdkClient.acceptOrgDevTerms(orgId)
    return res.body.accepted
  }
  ...
  async subscribeToServices (orgId, project, workspace, certDir, serviceProperties) {
    return this.subscribeToServicesWithCredentialType({
      orgId,
      project,
      workspace,
      certDir,
  ...
  async subscribeToServicesWithCredentialType ({
    orgId,
    project,
    workspace,
    certDir,
    serviceProperties,
    credentialType = LibConsoleCLI.JWT_CREDENTIAL // JWT_CREDENTIAL or OAUTH_SERVER_TO_SERVER
  }) {
    ...
    if (!credentialId) {
      if (credentialType === LibConsoleCLI.OAUTH_SERVER_TO_SERVER_CREDENTIAL) {
        credentialId = (await this.createOAuthServerToServerCredentials(
          orgId,
          project,
          workspace,
          `aio-${workspace.id}`,
  ...
  async getServicePropertiesFromWorkspace (orgId, projectId, workspace, supportedServices) {
    return this.getServicePropertiesFromWorkspaceWithCredentialType({
      orgId,
      projectId,
      workspace,
      supportedServices
  ...
  // pass in supportedServices to fix licenseConfigs
  async getServicePropertiesFromWorkspaceWithCredentialType ({
    orgId,
    projectId,
    workspace,
    ...
    let credential
    if (credentialType === LibConsoleCLI.OAUTH_SERVER_TO_SERVER_CREDENTIAL) {
      credential = await this.getFirstOAuthServerToServerCredentials(orgId, projectId, workspace)
    } else { // default is jwt
      credential = await this.getFirstEntpCredentials(orgId, projectId, workspace)
    }
