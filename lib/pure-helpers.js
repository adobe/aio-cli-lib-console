...
function mergeExtensionPoints (currPayload, updatePayload) {
  ...
  currPayload = clonedeep(currPayload)
  updatePayload = clonedeep(updatePayload)
  // set merged to currPayload
  const merged = currPayload
  // add new payload to merged
  Object.entries(updatePayload)
    .forEach(([endpointName, newEndpoint]) => {
      const oldEndpoint = currPayload[endpointName]
...
function removeExtensionPoints (currPayload, payloadToRemove) {
  currPayload = clonedeep(currPayload)
  payloadToRemove = clonedeep(payloadToRemove)
  const ret = currPayload
  Object.entries(payloadToRemove)
