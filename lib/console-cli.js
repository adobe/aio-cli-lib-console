...
class LibConsoleCLI {
  ...
  async promptForSelectProject (projects, data = { projectId: null }, options = { allowCreate: false }) {
    let selectedProject
    if (data.projectId) {
  ...
  async promptForSelectWorkspace (workspaces, data = { workspaceId: null }, options = { allowCreate: false }) {
  ...
  async getWorkspaceConfig (orgId, projectId, workspaceId, supportedServices) {
    ...
    spinner.stop()
    const enhancedJSON = helpers.enhanceWorkspaceConfiguration(json, supportedServices)
    logger.debug(`(Enhanced) Downloaded Workspace config: ${JSON.stringify(enhancedJSON, null, 2)}`)
