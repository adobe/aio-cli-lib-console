...
class ConsoleCLILib {
  ...
  async promptForServiceProperties (workspaceName, supportedServices) {
    ...
    const selectedServices = await prompt.promptMultiSelect(
      `Add Services to Workspace '${workspaceName}'`,
      serviceChoices
    )
  ...
  async createEnterpriseCredentials (orgId, project, workspace, certDir) {
    ...
    spinner.stop()
    console.log(`Key pair '${publicKeyFileName}, ${privateKeyFileName}' valid for '${CERT_VALID_DAYS}' days, has been created into the folder: ${projectCertDir}`)
