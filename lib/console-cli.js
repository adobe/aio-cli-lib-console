...
class LibConsoleCLI {
  ...
  async getExtensionPoints (org, project, workspace) {
    logger.debug(`getExtensionPoints - ${JSON.stringify([...arguments])}`)
    ...
    spinner.stop()
    logger.debug(`Extension Points: ${JSON.stringify(extensionPoints, null, 2)}`)
    return extensionPoints
  }

  async updateExtensionPoints (org, project, workspace, extensionPoints) {
    logger.debug(`updateExtensionPoints - ${JSON.stringify([...arguments])}`)
    ...
    spinner.stop()
    logger.debug(`updateExtensionPoints response: ${JSON.stringify(res, null, 2)}`)
    return res
  }
  ...
  async updateExtensionPointsWithoutOverwrites (org, project, workspace, extensionPoints) {
    logger.debug(`updateExtensionPointsWithoutOverwrites - ${JSON.stringify([...arguments])}`)
    const prevExtensionPoints = this.getExtensionPoints(org, project, workspace)
    const newExtensionPoints = helpers.mergeExtensionPoints(prevExtensionPoints, extensionPoints)
    const res = this.updateExtensionPoints(org, project, workspace, newExtensionPoints)
    return res
  }
}
